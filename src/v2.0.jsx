...
const ArrowUpTrayIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 mr-2">
    <path fillRule="evenodd" d="M11.47 2.47a.75.75 0 0 1 1.06 0l4.5 4.5a.75.75 0 0 1-1.06 1.06l-3.22-3.22V16.5a.75.75 0 0 1-1.5 0V4.81L8.03 8.03a.75.75 0 0 1-1.06-1.06l4.5-4.5ZM3 18.75a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75a.75.75 0 0 1-.75-.75Z" clipRule="evenodd" />
  </svg>
);
...
const InitialScreen = (props) => (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 p-6">
      ...
      <div className="space-y-4 w-full max-w-md"> {/* Reduced space-y for tighter buttons */}
       ...
       {props.userId && <p className="mt-1 text-xs text-gray-400">By: Thanh Bình with ❤️</p>}
    </div>
  );
                ...
                <fieldset className="border border-gray-600 p-4 rounded-lg">
                    ...
                    <div className="mt-2 space-y-4">
                        ...
                        <div className="flex items-center mt-3">
                         ...
                         <p className="text-xs text-gray-400 italic">Khi tick chọn, AI sẽ được khuyến khích tạo nội dung khiêu dâm, bạo lực cực đoan một cách chi tiết.</p>
                    </div>
                </fieldset>
            </div>
...
const GameplayScreen = ({ 
    ...
    isProcessingAction, handleGenerateSuggestedActions, isGeneratingSuggestedActions, exportGameToJson
}) => (
...
const App = () => {
  ...
  const saveGameProgress = async (gameIdToSave, story, currentChoices, fullStoryHistory, geminiHistory, currentKnowledge) => {
    ...
    try {
        ...
        // BƯỚC QUAN TRỌNG: Loại bỏ mọi giá trị 'undefined' để tránh lỗi Firebase
        const dataToSave = JSON.parse(JSON.stringify(rawData));
        
        // Vì JSON.stringify sẽ làm mất serverTimestamp(), chúng ta gán lại nó sau khi đã làm sạch
  ...
  const exportGameToJson = () => {
    ...
    downloadAnchorNode.setAttribute("download", `savegame_${gameSettings.characterName || 'adventure'}_${new Date().toLocaleDateString()}.json`);
    ...
    setModalMessage({
      show: true,
      title: "Thành công",
      content: "Đã xuất dữ liệu game ra tệp .json thành công!",
      type: "success"
    });
  };
  ...
  const importGameFromJson = (event) => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            // 1. Lấy dữ liệu settings từ file JSON (File của bạn có key là gameSettings)
            const settings = importedData.gameSettings || importedData.settings || {};
            const history = importedData.story || importedData.storyHistory || [];

            if (!settings.characterName) {
                throw new Error("Tệp JSON thiếu thông tin nhân vật (characterName).");
            }

            // 2. Cập nhật vào State của App để đồng bộ giao diện
            // Sửa lỗi hiển thị: Gán đúng các trường vào gameSettings
            setGameSettings({
                theme: settings.theme || "",
                setting: settings.setting || "",
                characterName: settings.characterName || "",
                characterPersonality: settings.characterPersonality || "",
                characterGender: settings.characterGender || "",
                characterBackstory: settings.characterBackstory || "",
                difficulty: settings.difficulty || "Normal",
                language: settings.language || "Vietnamese",
                allowNsfw: settings.allowNsfw || false
            });

            setStoryHistory(history);
            setKnowledgeBase(importedData.knowledgeBase || {});

            // 3. TỰ ĐỘNG LƯU LÊN CLOUD (Tạo bản save mới)
            const gameId = settings.characterName;
            const gameDocRef = doc(db, `artifacts/${appId}/users/${userId}/games`, gameId);
            
            const rawData = {
                currentStory: history.length > 0 ? history[history.length - 1].content : "",
                currentChoices: [],
                storyHistory: history,
                chatHistoryForGemini: importedData.chatHistoryForGemini || [],
                knowledgeBase: importedData.knowledgeBase || {},
                settings: settings, // Lưu nguyên cụm settings vào cloud
                updatedAt: serverTimestamp()
            };

            // Làm sạch dữ liệu undefined
            const cleanData = JSON.parse(JSON.stringify(rawData));
            cleanData.updatedAt = serverTimestamp();

            await setDoc(gameDocRef, cleanData, { merge: true });

            // 4. THÔNG BÁO VÀ Ở LẠI TRANG CHỦ
            // Không gọi setCurrentScreen('game'), giữ nguyên 'initial'
            setModalMessage({
                show: true,
                title: "Nhập File Thành Công",
                content: `Đã tạo bản lưu cho nhân vật "${gameId}". Bạn có thể tìm thấy trong mục 'Tải Game Đã Lưu'.`,
                type: "success"
            });
            ...
            setModalMessage({
                show: true,
                title: "Lỗi Tải Tệp",
                content: error.message || "Định dạng tệp không hợp lệ.",
                type: "error"
            });
        } finally {
            target.value = ''; 
