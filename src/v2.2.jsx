...
const App = () => {
  ...
  const handleFetchSuggestions = async (fieldType) => {
    ...
    if (fieldType === "theme") {
      promptText =
        "Hãy gợi ý 5 chủ đề độc đáo và hấp dẫn bằng tiếng Việt cho một trò chơi phiêu lưu bằng văn bản (text adventure game) theo phong cách tiểu thuyết mạng Trung Quốc. Mỗi chủ đề trên một dòng, không cần đánh số đầu dòng.";
  ...
  const callGeminiAPI = async (prompt, isInitialCall = false) => {
    ...
    try {
      ...
      if (
        ...
        const { story, choices: newChoices } =
  ...
  const importGameFromJson = (event) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        setStoryHistory(gameData.storyHistory || []);
        setChatHistoryForGemini(gameData.chatHistoryForGemini || []);
        const choices = gameData.currentChoices || [];
        setChoices(choices);
        ...
        setModalMessage({
          ...
          content: `Đã khôi phục hành trình của ${gameData.settings?.characterName}. Các lựa chọn hành động đã sẵn sàng!`,
          type: "success",
        });
      } catch (error) {
