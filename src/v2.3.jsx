...
const CharacterInfoModal = ({
  ...
  return (
    ...
    /></svg> Thông Tin Nhân Vật & Thế Giới
        </h3>
      ...
      <div className="bg-gray-800 p-3 md:p-5 rounded-xl shadow-xl mt-auto">
        ...
        {gameOver.isDead && (
          <div className="mt-8 p-6 bg-red-900/20 border border-red-500/50 rounded-xl text-center">
            ...
            <p className="text-gray-300 mb-4">{gameOver.reason || "Nhân vật của bạn đã tử trận."}</p>
            <button
...
const App = () => {
      ...
      while ((match = regex.exec(storyContent)) !== null) {
        ...
        try {
          if (tagType === "GAME_OVER") {
            ...
            newKnowledgeUpdates._isGameOver = { isDead: true, reason: parsed.Reason || parsed.reason || "Không rõ lý do" };
            continue;
          }
          if (
  ...
  const callGeminiAPI = async (prompt, isInitialCall = false, overrideGameId = null) => { //binh
    const effectiveApiKey = apiMode === "defaultGemini" ? "" : apiKey;
    if (apiMode === "userKey" && !effectiveApiKey) {
      setModalMessage({
        show: true,
    ...
    try {
      ...
      if (
        ...
        const { story, choices: newChoices, gameOver: newGameOver } = parseGeminiResponseAndUpdateState(rawText); 
        if (newGameOver) {
        ...
        setStoryHistory((prevStoryHistory) => { //binh
  ...
  const initializeGame = async () => {
    ...
    const initialPrompt = `
        ...
        16. QUY TẮC XỬ LÝ PHẦN THƯỞNG: Ngay khi dùng thẻ [QUEST_UPDATED: title="Tên Nhiệm Vụ", status="completed"], PHẢI thực hiện các hành động sau dựa trên reward đã ghi trong thẻ [QUEST_ASSIGNED: ...]:
            - Nếu là Kỹ năng/Vật phẩm: Phải mô tả cảnh nhân vật lĩnh ngộ hoặc nhận được vật phẩm đó trong nội dung truyện và cập nhật ngay vào thẻ [KNOWLEDGE_ADDS: playerSkills="..."] hoặc [KNOWLEDGE_ADDS: inventory="..."].
            - Nếu là Địa điểm/Thông tin: Phải mở khóa tình tiết mới trong diễn biến, buộc phải xuất hiện ít nhất một lựa chọn dẫn đến địa điểm hoặc sử dụng thông tin đó trong các lựa chọn tiếp theo.
            - Nếu là Chỉ số/Trạng thái: Dùng thẻ [KNOWLEDGE_ADDS: playerStatus="..."] để phản ánh sự thay đổi sức mạnh hoặc danh tiếng của người chơi ngay lập tức.
        ...
        17. Tạo 4-5 lựa chọn hành động rõ ràng, có ý nghĩa, đa dạng. Các lựa chọn phải phản ánh tính cách và mục tiêu của nhân vật, cũng như tình hình thực tế, các trạng thái và nhiệm vụ hiện tại.
        18. QUY TẮC TỶ LỆ THÀNH CÔNG & ĐỘ KHÓ ÁC MỘNG:
            - Với các lựa chọn rủi ro, hãy mô tả: (Tỷ lệ thành công: Cao/Trung Bình/Thấp/Rất Thấp. Rủi ro: ... Phần thưởng: ...). Tỷ lệ này phải bị ảnh hưởng bởi trạng thái, tính cách, trang bị, vật phẩm, kỹ năng của nhân vật và tình huống. Ví dụ: "1. Thử leo vách đá. (Tỷ lệ thành công: Thấp do ngươi đang [Bị Thương]. Rủi ro: Ngã nặng hơn. Phần thưởng: Tìm được lối tắt.)"
            - ĐẶC BIỆT VỚI ĐỘ KHÓ "ÁC MỘNG": 
                + Nếu người chơi chọn hành động có tỷ lệ thành công là "Thấp" hoặc "Rất Thấp", AI PHẢI cho nhân vật thất bại thảm hại.
                + Hậu quả của thất bại này là NHÂN VẬT CHÍNH SẼ CHẾT ngay lập tức.
                + Khi cái chết xảy ra, hãy viết một đoạn kết bi thảm, sau đó dùng thẻ [GAME_OVER: Reason="Mô tả lý do chết"] và không đưa ra thêm bất kỳ lựa chọn nào nữa.
            - Với các độ khó thấp hơn, thất bại chỉ dẫn đến chấn thương hoặc mất đồ nhưng câu chuyện vẫn tiếp tục.
        19. Lời thoại trong ngoặc kép, tên NV đứng trước. Suy nghĩ trong *suy nghĩ* hoặc _suy nghĩ_.
        20. Duy trì độ khó. Nhân vật có thể gặp bất lợi, thất bại nhưng câu chuyện vẫn tiếp diễn.
        21. Các thẻ [TAG:...] phải ở dòng riêng. Kết thúc phản hồi CHỈ bằng danh sách lựa chọn.
        ...
        23. CƠ CHẾ KẾT THÚC (DEATH LOGIC):
            - Nếu nhân vật chết (do chọn sai ở độ khó Ác Mộng hoặc do diễn biến logic), câu chuyện phải dừng lại hoàn toàn. 
            - Tuyệt đối không được "cứu" nhân vật bằng những tình tiết phi logic nếu đó là độ khó Ác Mộng.
    `;
  ...
  const handleChoice = (choiceText) => {
    ...
    const subsequentPrompt = `
        ...
        8.  CƠ CHẾ XÚC XẮC NGẦM (DICE ROLL): Với mỗi hành động nguy hiểm hoặc có tỷ lệ thành công, AI phải tự tính toán xác suất.
            - Tiếp tục duy trì mức độ thử thách. Các quyết định của người chơi có thể dẫn đến hậu quả nghiêm trọng. Luôn ghi nhớ và đề cập đến các yếu tố quan trọng liên quan đến nhân vật và thế giới. Đảm bảo tính logic của câu chuyện. 
            - Nếu kết quả là Thất bại: Tuyệt đối không được tìm cách cứu vãn bằng "may mắn bất ngờ". Hãy mô tả sự tàn khốc của thế giới.
            - Nếu nhân vật hết đường sống, bắt buộc dùng thẻ [GAME_OVER: Reason="Mô tả chi tiết lý do cái chết dựa trên bối cảnh"] và dừng mọi lựa chọn.
            - ĐẶC BIỆT VỚI ĐỘ KHÓ "ÁC MỘNG": 
                + Nếu người chơi chọn hành động có tỷ lệ thành công là "Thấp" hoặc "Rất Thấp", AI PHẢI cho nhân vật thất bại thảm hại.
                + Hậu quả của thất bại này là NHÂN VẬT CHÍNH SẼ CHẾT ngay lập tức.
                + Khi cái chết xảy ra, hãy viết một đoạn kết bi thảm, sau đó dùng thẻ [GAME_OVER: Reason="Mô tả lý do chết"] và không đưa ra thêm bất kỳ lựa chọn nào nữa.
  ...
  const loadGame = async (gameData) => {
    ...
    const currentGameOverStatus = gameData.gameOver || { isDead: false, reason: "" };
  ...
  const exportStoryToTxt = () => {
    try {
        const skillsString = knowledgeBase.playerSkills
            .map(skill => `▪ ${skill.Name}: ${skill.Description || "Không có mô tả"}`)
            .join('\n');
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-2xl max-w-md w-full p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Ủng Hộ Tác Giả</h3>
              ...
              <div className="bg-white p-3 rounded-xl mb-6 shadow-lg transform transition-transform hover:scale-105">
                ...
                />
                <p className="text-[10px] text-gray-500 mt-1 font-semibold uppercase">Quét mã để chuyển khoản nhanh</p>
              </div>
              ...
              <div className="w-full space-y-4 bg-gray-900/50 p-4 rounded-xl border border-gray-700 text-left">
                <div>
                  ...
                  <p className="text-white text-base font-semibold text-wrap">BIDV</p>
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase">Chủ tài khoản</label>
                  <p className="text-white font-medium uppercase text-sm">NONG THANH BINH</p>
                </div>
              </div>
